...
class OSSHook(BaseHook):
    ...
    def __init__(self,conn_id, remote_base_folder):
        ...
        (bucket_name, base_folder) = OSSHook.parse_oss_url(remote_base_folder)
        self.log.info("Init OSSHook with oss bucket: {}, base folder: {}".format(bucket_name, base_folder))
        extra_config = self.conn.extra_dejson
        oss_access_key_id = None
        if 'oss_access_key_id' in extra_config:
            oss_access_key_id = extra_config['oss_access_key_id']
        if 'oss_access_key_secret' in extra_config:
            oss_access_key_secret = extra_config['oss_access_key_secret']
        oss_region = 'cn-hangzhou'
        if 'oss_region' in extra_config:
            oss_region = extra_config['oss_region']
